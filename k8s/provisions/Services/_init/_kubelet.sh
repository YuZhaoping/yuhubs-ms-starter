...
kube_node_src_dir="$DEPLOY_HOME/${node_name}/kube/kubelet"

if [ -d "$kube_node_src_dir" ]; then
echo "--- init: [${node_name}] kubelet ---"


kube_common_src_dir="$DEPLOY_HOME/kube/kubelet"

cp_systemd_service "$kube_common_src_dir" "kubelet.service"
mk_service_workdir "$kube_common_src_dir/kubelet.service"
...
cp -vf $kube_node_src_dir/*.json "$kube_env_dest_dir/"
