...
kube_node_src_dir="$DEPLOY_HOME/${node_name}/kube/controller-manager"

if [ -d "$kube_node_src_dir" ]; then
echo "--- init: [${node_name}] kube-controller-manager ---"


kube_common_src_dir="$DEPLOY_HOME/kube/controller-manager"

cp_systemd_service "$kube_common_src_dir" "kube-controller-manager.service"
