...
kube_node_src_dir="$DEPLOY_HOME/${node_name}/kube/scheduler"

if [ -d "$kube_node_src_dir" ]; then
echo "--- init: [${node_name}] kube-scheduler ---"


kube_common_src_dir="$DEPLOY_HOME/kube/scheduler"

cp_systemd_service "$kube_common_src_dir" "kube-scheduler.service"
