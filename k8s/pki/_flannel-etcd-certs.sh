...
DEPLOY_DIR="${DEPLOY_HOME}/flannel"

CERTS_DIR=./certs/flannel
BUILD_CERTS_HOME="$BUILD_DIR/certs/flannel"
...
for cert_name in ${CERT_NAMES[@]}; do
  ...
  if [ ! -f "$BUILD_CERT_DIR/${cert_name}.pem" ]; then
    echo "--- Generate the flannel: \"${cert_name}\" cert ---";
  ...
  echo "--- Verify the flannel: \"${cert_name}\" cert ---";
...
#
  ...
  if [ ! -f "$DEPLOY_DIR/${cert_name}.key" ]; then
    echo "--- Deploy the flannel: \"${cert_name}\" cert ---";
